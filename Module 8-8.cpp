...
int main()
{
	...
	float percentDeactivateAntibiotic = 0.2f;

	// задаём время эксперимента
	int experimentTime = 20;
	...
	for (int i = 1; i <= experimentTime; i++) {
		...
		antibioticDrops -= antibioticDrops * percentDeactivateAntibiotic;
		std::cout << "После " << i << " минуты эксперимента бактерий осталось: " << numberBacteria << ". Концетрация атибиотика: " << antibioticDrops << "." << std::endl;
	}
